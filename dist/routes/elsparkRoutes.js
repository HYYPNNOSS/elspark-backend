...
const elspark_service_1 = require("../services/elspark.service");
const elspark_socket_1 = require("../sockets/elspark.socket");
const router = express_1.default.Router();
const elsparkService = new elspark_service_1.ElsparkService();
const tempDir = path_1.default.join(os_1.default.tmpdir(), 'elspark-temp');
...
const validateProfileIdBody = (req, res, next) => {
...
const validateProfileIdQuery = (req, res, next) => {
...
router.post('/collection/upload', upload.single('video'), async (req, res) => {
    const profileId = parseInt(req.body.profileId);
    if (!profileId || isNaN(profileId)) {
    ...
    try {
        ...
        const result = await elsparkService.uploadToCollection(profileId, req.file, title.trim(), description?.trim());
        if (req.app.get('io')) {
            const io = req.app.get('io');
            (0, elspark_socket_1.emitCoinUpdate)(profileId, result.newBalance, io);
        }
        res.status(201).json({
            success: true,
...
router.get('/collection', validateProfileIdQuery, async (req, res) => {
    try {
...
router.post('/collection/purchase/:videoId', validateProfileIdBody, async (req, res) => {
    try {
        ...
        if (req.app.get('io')) {
            const io = req.app.get('io');
            (0, elspark_socket_1.emitCoinUpdate)(profileId, result.buyerNewBalance, io);
            for (const update of result.ownerUpdates) {
                (0, elspark_socket_1.emitCoinUpdate)(update.profileId, update.newBalance, io);
            }
        }
        res.status(201).json({
            success: true,
...
router.post('/collection/post/:videoId', validateProfileIdBody, async (req, res) => {
    try {
        const { videoId } = req.params;
        const { profileId } = req.body;
        ...
        if (req.app.get('io')) {
            ...
            await (0, elspark_socket_1.broadcastQueueUpdate)(io);
            ...
            setImmediate(async () => {
                ...
                for (let i = 0; i < 3; i++) {
                    const started = await (0, elspark_socket_1.checkAndStartPlayback)(namespace);
                    if (started) {
...
router.delete('/collection/:videoId', validateProfileIdBody, async (req, res) => {
    try {
        const { videoId } = req.params;
        const { profileId } = req.body;
...
router.delete('/videos/:id', validateProfileIdBody, async (req, res) => {
    try {
        const { id } = req.params;
        const { profileId } = req.body;
        ...
        if (req.app.get('io')) {
            const io = req.app.get('io');
            (0, elspark_socket_1.broadcastQueueUpdate)(io);
        }
        res.json({
            success: true,
...
router.get('/live-tv/queue', async (req, res) => {
    try {
        const queue = await elsparkService.getQueue();
        res.json({
            success: true,
...
router.get('/live-tv/current', async (req, res) => {
    try {
        const current = await elsparkService.getCurrentVideo();
        res.json({
            success: true,
...
router.get('/live-tv/history', async (req, res) => {
    try {
...
router.get('/coins/balance', validateProfileIdQuery, async (req, res) => {
    try {
        const profileId = res.locals.profileId;
...
router.get('/coins/transactions', validateProfileIdQuery, async (req, res) => {
    try {
        const profileId = res.locals.profileId;
