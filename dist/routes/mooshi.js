...
${journalEntries.map((j) => `- ${j.entry}`).join('\n')}
...
router.post('/create-all-mooshis', async (req, res) => {
    try {
        const results = {
            successful: [],
            failed: []
        ...
        for (let mooshiNumber = 1; mooshiNumber <= 107; mooshiNumber++) {
            try {
                ...
                if (existing) {
                    results.failed.push({ mooshiNumber, reason: 'Already exists' });
                    continue;
                }
