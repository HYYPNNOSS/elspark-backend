...
exports.setupElsparkWebSocket = setupElsparkWebSocket;
exports.playNextVideo = playNextVideo;
exports.checkAndStartPlayback = checkAndStartPlayback;
exports.emitToUser = emitToUser;
exports.broadcastQueueUpdate = broadcastQueueUpdate;
exports.emitCoinUpdate = emitCoinUpdate;
exports.emitCollectionUpdate = emitCollectionUpdate;
...
let fillerVideoQueue = [];
let playedFillerVideoIds = [];
...
const activeElsparkUsers = new Map();
const recentChatMessages = [];
function setupElsparkWebSocket(io) {
    ...
    elsparkNamespace.on('connection', async (socket) => {
        ...
        socket.on('video:skip', async (data) => {
            try {
                await playNextVideo(elsparkNamespace);
            }
            catch (error) {
...
async function playNextVideo(namespace, retryCount = 0) {
    const MAX_RETRIES = 3;
...
async function loadInitialVideo(namespace) {
    try {
...
function startVideoSyncBroadcast(namespace) {
...
async function checkAndStartPlayback(namespace, retryCount = 0) {
    const MAX_RETRIES = 5;
...
function emitToUser(profileId, event, data, io) {
...
function emitCoinUpdate(profileId, newBalance, io) {
    emitToUser(profileId, 'user:coin_update', { balance: newBalance }, io);
}
function emitCollectionUpdate(profileId, io) {
